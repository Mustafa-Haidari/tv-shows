    ...
    item.addEventListener("click", (e) => {
      ...
      for (let item of currentItems) {
        if (idOfItem === item.show.id.toString()) {
          document
            .querySelector(".view-container .image")
            .setAttribute(
              "src",
              item.show.image.medium ? item.show.image.medium : "#"
            );
          document.querySelector(".view-container .name").innerText =
            item.show.name;
          document.querySelector(".view-container .genres").innerText =
            item.show.genres.length == 0 ? "Not specified" : item.show.genres;
          document.querySelector(".view-container .language").innerText =
            item.show.language;
          document.querySelector(".view-container .aired").innerText =
            item.show.aired;
        }
      }
      popupDisplay();
    ...
    item.addEventListener("click", (e) => {
      ...
      for (let item of localStorageData) {
        if (idOfItem === item.id.toString()) {
          ...
          document.querySelector(".view-container .genres").innerText =
            item.genres.length == 0 ? "Not specified" : item.genres;
          document.querySelector(".view-container .language").innerText =
            item.language;
          document.querySelector(".view-container .aired").innerText =
            item.aired;
...
const makeImages = () => {
  currentItems.map((result) => {
    ...
    img.setAttribute(
      "src",
      result.show.image.medium ? result.show.image.medium : "#"
    );
